...
def compute_ld_fsc(embeddings, target, neighbors):
...
def levenshtein_fsc_full(targets2neighbors, embedding_space, threads=8):
    ...
    with mp.Pool(threads) as pool:
        outputs = pool.imap(
            _mp_compute_ld_fsc, ((embedding_space, target, neighbors) for target, neighbors in targets2neighbors.items())
        )
