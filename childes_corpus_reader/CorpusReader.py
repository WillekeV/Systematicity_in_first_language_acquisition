from nltk.corpus.reader import CHILDESCorpusReader, NS
from nltk.corpus.reader.xmldocs import ElementTree

# this file specifies a modified ChildesCorpusReader class, which will automatically tag each word with its' PoS tag
# the class is also able to handle clitics
...
class ModifiedCHILDESCorpusReader(CHILDESCorpusReader):
    ...
    """
    Modified (cleaner) '_get_words' method of the CHILDES corpus reader from the NLTK to fetch clitics as separate
    lexical entries if you retrieve stemmed sentences, e.g. by calling 'corpus_reader.tagged_sents(stem=True)'.
    """
    
    def _get_words(self, fileid, speaker, sent, stem, relation, pos, strip_space, replace):

        # ensure we have a list of speakers
        if isinstance(speaker, list) and speaker != 'ALL':
            speaker = [speaker]
